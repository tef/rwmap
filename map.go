...
func (m *RWMap) markDeleted(key any, value *mapEntry) {
	...
	// in theory, another thread could see our deleted value inside big
	// and add a new value, so we insert our record only if it's missing
